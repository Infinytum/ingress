...
run:
	...
	@env POD_NAME="localhost-dev" env POD_NAMESPACE="default" go run main.go --kube-config ~/.kube/config $(if $(class-name),--class-name=$(class-name),"") $(if $(namespace),--namespace=$(namespace),"")
